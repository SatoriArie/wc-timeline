...
export interface Zone {
  ...
  images: string[];
  /** Координаты пина на карте (px в системе карты-подложки); нет → авто-раскладка */
  mapX?: number;
  mapY?: number;
  /** Уровень зума при клике по карточке зоны (кадр); нет → дефолт */
  mapZoom?: number;
  /** Контур-обводка зоны: точки [x,y] в координатах карты */
  poly?: [number, number][];
}
